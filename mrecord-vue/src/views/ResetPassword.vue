  ...
  <div class="reset-page">
    ...
    <div class="reset-container">
      ...
      <div v-if="invalidToken" class="form-card error-card">
        ...
        <p class="status-desc">密码重置链接无效或已过期，请重新申请。</p>
        <button class="submit-btn" @click="goLogin">返回登录</button>
      </div>

      <!-- 重置密码表单 -->
      <div v-else-if="!success" class="form-card">
      ...
      <div v-else class="form-card success-card">
        ...
        <h3 class="status-title">密码已重置</h3>
        <p class="status-desc">您的密码已成功修改，请使用新密码登录。</p>
...
.card-desc { font-size: 13px; color: #999; line-height: 1.5; margin-bottom: 24px; }
...
.eye-btn {
  ...
  width: 36px; height: 36px; border-radius: 10px; color: #c0a080;
  cursor: pointer; transition: color 0.2s; background: none; border: none; padding: 0; flex-shrink: 0;
}

...
public class SysExportTaskController {
     ...
     */
    @PostMapping("/export")
    public Result<String> export(ExportBookDTO dto) {
    ...
    public Result<List<SysExportTask>> list() {
        String userId = UserContext.getUserId();
        log.info("查询当前用户的导出任务列表[/exportTask/list]请求传参：{}", userId);
        return Result.success(sysExportTaskService.listMyTasks(userId));
    }
}
